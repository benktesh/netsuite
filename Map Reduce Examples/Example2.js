    ...
    function (search, record, format) {
        function getInputData() {
            return search.create({
                type: 'item',
                filters: [search.createFilter({ name: 'inactive', operator: search.Operator.IS, values: false })],
                columns: [
        ...
        function createPed(priceData, effectiveDate) {
            try {
                var newPrice = Math.round(priceData.price * (1 + .02));
        ...
        function deletePed(itemId, effectiveDate) {
                ...
                var objSearch = search.create({
                    ...
                    filters: [
                        ...
                        search.createFilter({
                            ...
                            values: format.format({ value: effectiveDate, type: format.Type.DATE })
                        })
                    ],
                    columns: [
                        search.createColumn({
            ...
            } catch (ex) { log.error({ title: "Error Deleting", details: ex }); }
        }
        ...
        function reduce(context) {
            try {
                ...
                priceData.forEach(
                    function (pd) {
                        var pdObject = JSON.parse(pd);
                        createPed(pdObject, effectiveDate);
                    });
                context.write(context.key, true);

            } catch (ex) {
                log.error({ title: "Error in item: " + context.key,details: JSON.stringify(context.values) + ex });
            }
        }
